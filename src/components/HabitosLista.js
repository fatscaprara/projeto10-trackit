...
export default function HabitosLista({ id, nome, dias }) {
  const { token, receberHistorico, receberHabitosHoje, receberHabitosDiarios } =
    ...
    <CaixaHabito>
      ...
      <div>
        {dias.includes(0) ? (
          ...
          <NaoSelecionado>D</NaoSelecionado>
        )}
        {dias.includes(1) ? (
          <Selecionado>S</Selecionado>
        ) : (
          <NaoSelecionado>S</NaoSelecionado>
        )}
        {dias.includes(2) ? (
          <Selecionado>T</Selecionado>
        ) : (
          <NaoSelecionado>T</NaoSelecionado>
        )}
        {dias.includes(3) ? (
          <Selecionado>Q</Selecionado>
        ) : (
          <NaoSelecionado>Q</NaoSelecionado>
        )}
        {dias.includes(4) ? (
          <Selecionado>Q</Selecionado>
        ) : (
      ...
      <img
        ...
        onClick={() => {
          const confirmar = window.confirm(
            "Deseja mesmo apagar este hábito da sua lista?"
          );
          if (confirmar) {
            apagar();
          }
