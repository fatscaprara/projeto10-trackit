...
export default function Historico() {
  ...
  function mostrarTituloDosHabitosDoDiaClicado() {
    ...
    return listaDeHabitos.length > 0 ? (
      <TextoPrincipal>
        Hábitos do dia {dayjs(habitoDoDia).locale("pt-br").format("DD/MM")}
      </TextoPrincipal>
    ) : (
      <></>
    );
      ...
      <div className="habits-list">
        {listaDeHabitos.map((habito, indice) => (
          <div key={indice} className="habits">
            <TextoAtividade className="habito-name">
              {habito.name}:{" "}
            </TextoAtividade>
            {habito.done ? (
              <TextoFeito>Feita</TextoFeito>
            ) : (
              <TextoNaoFeito>Não Feita</TextoNaoFeito>
            )}
          </div>
        ))}
      </div>
